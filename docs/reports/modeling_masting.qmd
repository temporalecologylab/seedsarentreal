...
nonmasting <- sapply(1:10000, function(x){
  ifelse(rbinom(1, 1, theta) == 1, 0, MASS::rnegbin(1000, 25, 5))
})
...
text(x, rep(1.33, 5), labels = c('$\\beta \\cdot X_\\text{climate}\\{t\\}$', '...', '$\\beta \\cdot X_\\text{climate}\\{t\\}$', '$\\beta \\cdot X_\\text{climate}\\{t\\}$', '...'), col = 'grey30', cex = 0.85)
arrows(x, 1.27, x, 1.05, col = 'grey40', lwd=1, length = 0.07, lty = 2)
