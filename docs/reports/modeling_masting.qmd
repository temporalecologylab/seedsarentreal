...
Each state $z_{i+1}$ is dependent **only on the preceding state** $z_{i}$ (which is called the Markovian assumption). These states are latent, because we observe only the seed counts.\
With some parameters $\theta$, the probability of observing some sequences of the two first states is:
...
\end{bmatrix}
$$
The probability $p(z_{2})$ is then obtain with the product:
...
Again, the transition matrix corresponds to only two parameters, for example:
$$
M =
\begin{bmatrix}
...
In summary, we have 3 parameters to infer: the initial probability of masting $\tau_0= p(z_{1}=\text{\textcolor{mid}{on}})$, the probability of transitioning from an \textcolor{teal}{off-year} to an \textcolor{teal}{on-year} $\tau_{\text{\textcolor{teal}{off}} \rightarrow \text{\textcolor{mid}{on}}}$, and the probability of staying into an \textcolor{teal}{on-year} state $\tau_{\text{\textcolor{mid}{on}} \rightarrow \text{\textcolor{mid}{on}}}$. 
