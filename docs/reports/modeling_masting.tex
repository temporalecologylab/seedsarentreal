...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, left=2mm, bottomrule=.15mm, breakable, colframe=quarto-callout-tip-color-frame, colback=white, opacityback=0, opacitybacktitle=0.6, bottomtitle=1mm, toptitle=1mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{The ecological motivation}, coltitle=black, titlerule=0mm, arc=.35mm, toprule=.15mm, rightrule=.15mm, leftrule=.75mm]
...
\(z_{i}\) (which is called the Markovian assumption). These states are
latent, because we observe only the seed counts.\\
With some parameters \(\theta\), the probability of observing some
...
\paragraph{Transition between states}\label{transition-between-states}
...
Again, the transition matrix corresponds to only two parameters, for
example: \[
M =
\begin{bmatrix}
...
\paragraph{Seed production parameters}\label{seed-production-parameters}

Now, what about the parameters of the observational model that links the
latent state to the observed seed count?\\
