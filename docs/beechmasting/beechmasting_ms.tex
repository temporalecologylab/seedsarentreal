...
Following the paradigm that masting is defined by of `boom and bust' reproduction, our model identified high and low reproductive years as alternative latent states (Fig. \ref{fig:overview}A-B). In high (`boom') reproductive years, individual trees produced 175 seeds on average (90\% uncertainty range: 167 -- 184), while in low (`bust') reproductive years they produced 31 seeds on average (24 -- 38). Seed number per tree in low and high reproductive years, however, overlapped significantly (both distributions spanned seed numbers of XX-YY). The difference between these two states thus occurs mostly in the opposite extremes---years in the high reproduction latent state included very high seed counts, which would almost never be possible in the low reproduction state, while years in the low reproduction state were characterized by extremely low counts (between 0 and 10 seeds) almost never seen in the high reproductive state (Fig. \ref{fig:overview}A). 
...
Transitioning between reproductive states (high or low) across years depended on a tree's state in the previous year (Fig. \ref{fig:overview}C).  In a year with average climatic conditions (summer temperatures around 19.7\celsius), a tree in a low reproductive state was most likely to transition to a high reproductive state the next year, with a probability of 66\% (60 -- 71\%), with a 1.9 times lower probability (34\%, 29 -- 40\%) of staying in a low reproductive state (Fig. \ref{fig:overview}C).
