...
These constraints may limit how climate can affect seed production. Ideal climatic conditions may trigger higher seed production for individual trees only when they are not already in a high reproduction ('on-year') state. Warm summers would then not affect all trees equally, but only those in in low reproductive state (off-years) and would thus drive an underlying transition from an off-year to an on-year, thus determining the tree reproductive state the year before seed production. Spring frosts occur later in the cycle and may then only impact trees once a higher reproductive state is already established, [make new sentence] and should affect the potential seed production by damaging flowers. [These individual-tree level dynamics would then scale up to produce population synchrony that defines masting through a combination and reproductive constraints.] % Masting may thus depend on the combination of reproductive constraints, which operate at the individual-level, and climatic cues that operate over larger scales. 
...
Such an approach would let you forecast and would be nifty!
