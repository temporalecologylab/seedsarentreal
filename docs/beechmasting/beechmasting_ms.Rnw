...
% emwFeb2: MOVE UP?
Synchronization depended both on previous reproductive states and climate. This mechanism provides a simple physiological explanation for previous findings that temperature difference between the two previous summers mattered for masting ($\Delta T$ model; \citealp{Kelly2012}). Rather than complex molecular pathways such as `epigenetic summer memory' \citep{Samarth2020}, a cold summer followed by a warm summer generates synchrony simply because cold temperatures leave most trees in a low-reproduction state, allowing them to respond consistently to following warm temperatures that promote floral induction (Fig. \ref{fig:synchrony}B).
% Some ideas moved from the intro:
