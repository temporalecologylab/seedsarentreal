...
% <!-- % \item ... -->
% <!-- % \end{itemize} -->
%emwFeb5:  START here ... now we can have a bit of a lit review paragraph but it needs to tie tightly to previous paragraph .... Start with something like: These results highlight how biological constraints on plant reproduction may limit the runaway effects of climate change sometimes proposed.... and then edit (this paragraph needs some work, I did not work on it, I just added a potential ending to it and looked ahead).
...
%emwFeb5: Maybe end on ...
Despite these constraints, synchrony across populations still appears to go down in the last two decades (Fig. \ref{fig:overview}D). 
