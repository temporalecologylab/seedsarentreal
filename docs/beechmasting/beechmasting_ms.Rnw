...
% <!-- % \end{itemize} -->
Reproductive dynamics are impacted by climate. In a warm summer (\Sexpr{kq$warmsummertemp}$^\circ$C), trees in a low-reproduction state are \Sexpr{round(kq$probability_ratio['50%'],1)} times (95\%CI \Sexpr{round(kq$probability_ratio['2.5%'],1)}-\Sexpr{round(kq$probability_ratio['97.5%'],1)}) more likely to transition to a high-reproduction state than during cold summers (\Sexpr{kq$coldsummertemp}$^\circ$C, Fig. \ref{fig:climate}A). Once in a high-reproduction state, a three-fold increase of growing-degree days until last frost---a metric for late spring frost risk---can decrease seed production by \Sexpr{round(kq$frost_decrease['50%'],1)}\%  (95\%CI \Sexpr{round(kq$frost_decrease['2.5%'],1)} to \Sexpr{round(kq$frost_decrease['97.5%'],1)}\%, Fig. \ref{fig:climate}B). However, warmer spring conditions have no effects on seed production (Supp. Fig.). 
...
However, even with a significant and continuous summer warming ($+7^\circ$C in one century), only \Sexpr{round(kq$mastingtrees_warmfuture['50%'],1)}\%  (95\%CI \Sexpr{round(kq$mastingtrees_warmfuture['2.5%'],1)} to \Sexpr{round(kq$mastingtrees_warmfuture['97.5%'],1)}\%) of trees in a population would mast in any given year (Fig. \ref{fig:future}). % change 'every year' to 'in any given year' because it's not always the same trees... Is it clearer now?
