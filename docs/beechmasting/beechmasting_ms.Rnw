...
% Next paragraph...
Here we develop a predictive model that builds from climatic effects on individual trees constrained by their reproductive cycle (through two latent states). Our approach easilly scales up these individual dynamics to population-level behavior, to investigate how climate interacts with individual constraints to shape masting. Using seed data collected in beech forests of England since 1980, we first tested whether individual trees show evidence for alternative reproductive states that could lead to masting at the population level. Then, we examined how synchrony might arise from the combination of reproductive constraints, at the individual-level, and climatic cues that operate over larger scales. Finally, we forecasted how future warming may affect both tree-level seed production and population-level synchrony. 
% What do we do here? 
...
More broadly, understanding changes in reproductive dynamics will require replacing masting within a wider life-cycle context (CITE). Seed production represents only the first step toward forest regeneration, and climate change may lead into failures in seedling establishment and tree recruitment \citep{Petrie2023}, even in forests where seed production remains synchronous.
