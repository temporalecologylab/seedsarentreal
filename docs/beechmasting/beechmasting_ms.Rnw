...
Unfavorable or favorable summer conditions could act as a  hinge point to synchronize individual reproductive cycles within a population, and this synchrony may then persist over several years.
...
These individual tree-level states scale up to produce population-level seed production that we found was relatively synchronous---further supporting the general definition of masting (CITES): years of high population-level seed production---mast years---were almost always separated by periods of low seed production (Fig. \ref{fig:overview}D). %emwFeb2: check -- is 'almost always' accurate?

Transitioning between reproductive states depended on the state (high or low).  In a year with average climatic conditions (GIVE mean temp here? Or some quick definition of what is average climate in parentheses), a tree in a low reproductive state is most likely to transition to a high reproductive state the next year, with a probability of \Sexpr{round(kq$average_transition['50%'],2)} (90\%CI \Sexpr{round(kq$average_transition['5%'],2)}-\Sexpr{round(kq$average_transition['95%'],2)}), with a 3X lower probability [make Sexpr] \Sexpr{round(kq$average_persistence['50%'],2)} (90\%CI \Sexpr{round(kq$average_persistence['5%'],2)}-\Sexpr{round(kq$average_persistence['95%'],2)}) of staying in a low reproductive state (Fig. \ref{fig:overview}C).
%emwFeb2: The probabilities scale to 1? If so, we should make that clear or somehow help the audience interpret these #s better... Could we say 66% probability? Or in 66% of years?  
[Can we next add the rest of the probabilities? What happens in a HIGH state? We probably could address this here ... something like: Following constraints of reproductive biology (re-explain in parentheses quickly) we assumed trees in a high reproductive state would have to transition to low the following year (prob equals 1?), which we found strong support for when we relaxed this assumption (give \#s or ref supp?).] We found transitioning between states, however, was also strongly dependent on climate.

%emwFeb2: I think that I included all of this above now, so you can cut it (if you agree). 
Using these data and our model, we identified two latent states at the individual tree level, similar to those deserved for masting (Fig. \ref{fig:overview}A-B). %emwJan23: Not sure what deserved means here ... maybe start simpler -- we identified years as high or low seed production, as commonly hypothesized for masting. Then I could give more detail on these two latent states and some discussion -- for example, how much do they overlap? How different are they? How does this compare to the literature? Right now this paragraph needs more D of the R&D. 
%emwJan23: The below (and further below) is showing up as almost all commented out for me (because of %) so hard to edit ... how is it working for you?
Under average climatic conditions, a tree has a probability of \Sexpr{round(kq$average_transition['50%'],2)} (90\%CI \Sexpr{round(kq$average_transition['5%'],2)}-\Sexpr{round(kq$average_transition['95%'],2)}) of transitioning from a low-reproduction to a high-reproduction state, and a lower probability of \Sexpr{round(kq$average_persistence['50%'],2)} (90\%CI \Sexpr{round(kq$average_persistence['5%'],2)}-\Sexpr{round(kq$average_persistence['95%'],2)}) of remaining in that state the following year (Fig. \ref{fig:overview}C). Alternation between these states at the tree level generates high variability in seed production at the population scale (Fig. \ref{fig:overview}D). 

%emwFeb2:
% START here on CLIMATE ... (and be sure to include the probabilities in parentheses so I can better understand what you mean with 2X etc.).
% After that paragraph we may be able to move up the paragraph after % emwFeb2: MOVE UP? (below) ... not sure and either way we need to re-write it to explain more clearly first how cues drive synchronization (and give some synchronization results in there) ... 
...
% <!-- % \end{itemize} -->
%emwJan23:  less 'our model' and more 'we find' or similar, also less PV. I also think we could refer to 'years' more than 'states' -- it's just jargon here since our model encodes the states as different individual tree years (this distinction you could sneak into first paragraph)
%emwJan23: This paragraph also needs more D in the R and D ... and sneak in your methods (how did we come up with 23 C?). This is an R&D so you can say more: similar to many previous studies we found climate affected reproduction ...
Reproductive dynamics are impacted by climate. In a warm summer (\Sexpr{kq$warmsummertemp}\celsius), trees in a low-reproduction state are \Sexpr{round(kq$probability_ratio['50%'],1)} times (90\%CI \Sexpr{round(kq$probability_ratio['5%'],1)}-\Sexpr{round(kq$probability_ratio['95%'],1)}) more likely to transition to a high-reproduction state than during cold summers (\Sexpr{kq$coldsummertemp}\celsius, Fig. \ref{fig:climate}A). Once in a high-reproduction state, a three-fold increase of growing-degree days until last frost---a metric for late spring frost risk---can decrease seed production by \Sexpr{round(kq$frost_decrease['50%'],1)}\%  (90\%CI \Sexpr{round(kq$frost_decrease['5%'],1)} to \Sexpr{round(kq$frost_decrease['95%'],1)}\%, Fig. \ref{fig:climate}B). However, warmer spring conditions have no effects on seed production (Supp. Fig.). 
...
%emwJan23: Below you flip to your results but it's not at all clear to the reader. 
However, even with a significant and continuous summer warming (7\celsius~in one century), only \Sexpr{round(kq$mastingtrees_warmfuture['50%'],1)}\%  (90\%CI \Sexpr{round(kq$mastingtrees_warmfuture['5%'],1)} to \Sexpr{round(kq$mastingtrees_warmfuture['95%'],1)}\%) of trees in a population would mast in any given year (Fig. \ref{fig:future}). % change 'every year' to 'in any given year' because it's not always the same trees... Is it clearer now?

%emwJan23: The paragraph should probably focus on how we do not see breakdown (pulling from last paragraph perhaps) and why, with just an aside of what it would take to get breakdown. As written now it focuses a lot on the supp figure without fully explaining and digesting the main text figure for the audience. 
Since our initial model allowed summer temperatures to affect only transitions from low- to high-reproduction state, we additionally tested whether warm summer influences the probability of persistence in a high-reproduction state and found a weak effect (Supp. Fig.). Even with this additional effect, trees cannot get stuck indefinitely in a highly reproductive state. On the contrary, a breakdown because of climate change would require summer warming to have a similar impact on both transition and persistence into a high-reproduction year --- an hypothesis not supported by the data we used here (Fig. \ref{fig:future}).
...
%emwJan23: I would move this up to the cues section of the R&D. We need to add to the R&D some of the points you have in the intro currently, including that cold summers/frosts could drove synchronization so I think if you move this point up you can start to build that finding in there and come back to it later as you look at shifts in synchrony across and within populations. 
% emwFeb2: MOVE UP?
