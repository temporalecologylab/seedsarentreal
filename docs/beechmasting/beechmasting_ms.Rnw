...
%emwFeb3: Moving this text I had above down, as seems best to focus on individual trees THEN populations, I think. 
%emwFeb3:  START here ... I think we can merge the two bits here into a first paragraph that bridges to population dynamics ... 
Individual tree-level reproductive states (high or low) scaled up to produce population-level seed production that was relatively synchronous, further supporting the general definition of masting (CITES): years of high population-level seed production---mast years---were almost always separated by periods of low seed production (Fig. \ref{fig:overview}D). We distinguished two spatial scales of synchrony: within population and across populations. Within populations, \Sexpr{round(kq$withinstand_synchrony['50%']*100,0)}\%  (\Sexpr{round(kq$withinstand_synchrony['5%']*100,0)} -- \Sexpr{round(kq$withinstand_synchrony['95%']*100,0)}\%) of trees shared the same reproductive state on average each year---without considering the reproductive state observed in other populations. Synchrony across populations---i.e. the proportion of trees in the same state across all populations---was only slightly lower (\Sexpr{round(kq$acrossstands_synchrony['50%']*100,0)}\%, \Sexpr{round(kq$acrossstands_synchrony['5%']*100,0)} -- \Sexpr{round(kq$acrossstands_synchrony['95%']*100,0)}\%), likely because populations were not widely separated spatially (Supp. Mat.).

Reproductive constraints alone are not sufficient to explain this synchronization, and individual reproduction cycles must be synchronized by climatic cues (CITE). Synchrony can emerge at the population level only if most individual trees are in a low reproductive state and transition together into a high reproductive state. Our findings indicate that cold summer could act as a hinge point to synchronize individual cycles within a population: cold temperatures reduce the probability of transition to a high reproductive state and leave most trees in a low reproductive state (Fig. \ref{fig:climate}A). Consequently, when a cold summer is followed by a warm summer, most trees respond consistently to the warm temperatures that promote floral induction, and transition together into a high reproductive state (FIGURE DELTA T). This generate a mast year at the population scale. The interplay between individual-level constraints and population-level climatic cues provides a physiological mechanism for reproductive synchrony that can persist over multiple years, and offers a simple explanation for previous findings that masting depends on the temperature difference between successive summers (the $\Delta T$ model; \citealp{Kelly2012}).
% I was referring before to `epigenetic summer memory' \citep{Samarth2020} to say it's bullshit, I don't know if we should leave it?
