...
for(id in 1:ids){
  ...
  params <- rbind(params, data.frame(rho0, lambda1, psi1, lambda2, tau_m_nm, tau_nm_m))
...
ggplot() +
  ...
  geom_point(data = unique(lambda2[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'white', shape = '-',
             size = 10) +
  geom_point(data = unique(lambda2[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'red', shape = '-',
             size = 6) +
  theme_bw() + 
  theme(panel.grid = element_blank(),
...
ggplot() +
  geom_boxplot(data = lambda1, aes(y = samp, x = t, group = t), outliers = FALSE, width = 0.5,
               fill = 'grey90', color = 'grey80') +
  geom_point(data = unique(lambda1[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'white', shape = '-',
             size = 10) +
  geom_point(data = unique(lambda1[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'red', shape = '-',
             size = 6) +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        legend.position = 'none',
...
ggplot() +
  stat_summary(
    data = tau_nm_m, aes(y = samp, x = t, group = t),
    fun.min = function(z) { quantile(z,0.1)},
    fun.max = function(z) { quantile(z,0.9)},
    fun = median, color = 'grey80') +
  geom_point(data = unique(tau_nm_m[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'white', shape = '-',
             size = 12) +
  geom_point(data = unique(tau_nm_m[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'red', shape = '-',
             size = 8) +
  theme_bw() + 
  theme(panel.grid = element_blank(),
        legend.position = 'none',
...
ggplot() +
  stat_summary(
    data = lambda2, aes(y = samp, x = t, group = t),
    fun.min = function(z) { quantile(z,0.1)},
    fun.max = function(z) { quantile(z,0.9)},
    fun = median, color = 'grey80') +
  geom_point(data = unique(lambda2[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'white', shape = '-',
  ...
  theme(panel.grid = element_blank(),
        legend.position = 'none',
        plot.margin = margin(r = 5, l = 5, t=5),
        axis.text.x = element_blank(), axis.title.x = element_blank(),
        axis.ticks.x = element_blank()) +
  labs(y = TeX(r"( $\lambda_{masting}$ )"))
...
ggplot() +
  stat_summary(
    data = lambda1, aes(y = samp, x = t, group = t),
    fun.min = function(z) { quantile(z,0.1)},
    fun.max = function(z) { quantile(z,0.9)},
    fun = median, color = 'grey80') +
  geom_point(data = unique(lambda1[,c('sim', 't')]), aes(y = sim, x = t, group = t), color = 'white', shape = '-',
