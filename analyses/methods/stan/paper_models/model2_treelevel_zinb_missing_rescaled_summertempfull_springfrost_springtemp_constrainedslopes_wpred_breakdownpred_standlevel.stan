...
model {
  ...
  beta_lambda2_frost ~ normal(0, log(1.2) / 2.57); # 20% relative change
  beta_lambda2_spring ~ normal(0, log(1.2) / 2.57); # 20% relative change with one unit of frost risk
  psi2 ~ normal(0, 5 / 2.57); 
  beta_nm_m ~ normal(0, 0.6 / 2.57); # ~20% relative change at tau0 = 0.5
  beta_m_m ~ normal(0, 0.6 / 2.57); # ~20% relative change at tau0 = 0.5
...
generated quantities {
  ...
  for (t in 1:N_trees) {
    ...
    for (rn in 2:N_max_years) {
      ...
      real tau_m_m = inv_logit(logit(tau_m_m0) + beta_m_m * (prevsummer_temps_tree[n+1]-summertemp0));
      matrix[2, 2] Gamma = [ [1 - tau_nm_m, tau_nm_m],
                           [1 - tau_m_m, tau_m_m] ];
      ...
      tau_m_m = inv_logit(logit(tau_m_m0) + beta_m_m * (prevsummer_temps_tree[n+1]-summertemp0));
  ...
  for(newt in 1:N_newtrees){
    ...
    for (n in 1:N_max_newyears) {
      ...
      states_new[n] = categorical_rng(lambda);
      
      states_new_plot[newtree_stand_idxs[newt], n] += states_new[n]-1;
      
      if(states_new[n] == 1) {
        if(bernoulli_rng(theta1)){
          seed_counts_new[n] = 0;
        }else{
          seed_counts_new[n] = neg_binomial_2_alt_log_safe_rng(log(lambda1), psi1);  //neg_binomial_alt_rng(lambda1, psi1)
        }
      }
      else if(states_new[n] == 2) {
        real loglambda2 = log(lambda20) 
        + beta_lambda2_frost * (gdd_lastfrost_tree[n]-frostgdd0) 
        + beta_lambda2_spring * (spring_temps_tree[n]-springtemp0);
      
        seed_counts_new[n] = neg_binomial_2_alt_log_safe_rng(loglambda2, psi2); // neg_binomial_alt_rng(lambda2, psi2) 
      }
      
      seed_counts_new_plot[newtree_stand_idxs[newt], n] += seed_counts_new[n];
    }
  }
  
  // Predict for new trees-in a case where beta_m_m is very high, like 0.6?
  real fixed_beta_m_m = 0.6;
  
  array[N_newstands, N_max_newyears] int states_new_brkd_plot = rep_array(0, N_newstands, N_max_newyears);
  array[N_newstands, N_max_newyears] int seed_counts_new_brkd_plot = rep_array(0, N_newstands, N_max_newyears);
  ...
  for(newt in 1:N_newtrees){
    ...
    array[N_max_newyears] int states_new_brkd;
    array[N_max_newyears] int seed_counts_new_brkd;
    ...
    for (n in 1:N_max_newyears) {
      ...
      // int global_idx = n + (N_max_newyears)*(newt-1);
      real tau_nm_m = inv_logit(logit(tau_nm_m0) + beta_nm_m * (prevsummer_temps_tree[n]-summertemp0));
      real tau_m_m = inv_logit(logit(tau_m_m0) + fixed_beta_m_m * (prevsummer_temps_tree[n]-summertemp0));
      matrix[2, 2] Gamma = [ [1 - tau_nm_m, tau_nm_m],
                           [1 - tau_m_m, tau_m_m] ];
      ...
      vector[2] lambda = alpha[n] / sum(alpha[n]);
      states_new_brkd[n] = categorical_rng(lambda);
      
      states_new_brkd_plot[newtree_stand_idxs[newt], n] += states_new_brkd[n]-1;
      
      if(states_new_brkd[n] == 1) {
        if(bernoulli_rng(theta1)){
          seed_counts_new_brkd[n] = 0;
        }else{
          seed_counts_new_brkd[n] = neg_binomial_2_alt_log_safe_rng(log(lambda1), psi1);  //neg_binomial_alt_rng(lambda1, psi1)
        }
      }
      else if(states_new_brkd[n] == 2) {
        real loglambda2 = log(lambda20) 
        + beta_lambda2_frost * (gdd_lastfrost_tree[n]-frostgdd0) 
        + beta_lambda2_spring * (spring_temps_tree[n]-springtemp0);
      
        seed_counts_new_brkd[n] = neg_binomial_2_alt_log_safe_rng(loglambda2, psi2); // neg_binomial_alt_rng(lambda2, psi2) 
      }
      
      seed_counts_new_brkd_plot[newtree_stand_idxs[newt], n] += seed_counts_new_brkd[n];
    }
  }
}
