...
generated quantities {
  ...
  array[Nnew] int states_new_brkd;
  array[Nnew] int seed_counts_new_brkd;
  ...
  for(newt in 1:N_newtrees){
    ...
    for (n in 1:N_max_newyears) {
      ...
      vector[2] lambda = alpha[n] / sum(alpha[n]);
      states_new_brkd[global_idx] = categorical_rng(lambda);
      
      states_new_brkd_plot[newtree_stand_idxs[newt], n] += states_new_brkd[global_idx]-1;
      
      if(states_new_brkd[global_idx] == 1) {
        if(bernoulli_rng(theta1)){
          seed_counts_new_brkd[global_idx] = 0;
        }else{
          seed_counts_new_brkd[global_idx] = neg_binomial_2_alt_log_safe_rng(log(lambda1), psi1);  //neg_binomial_alt_rng(lambda1, psi1)
        }
      }
      else if(states_new_brkd[global_idx] == 2) {
        real loglambda2 = log(lambda20) 
        + beta_lambda2_frost * (gdd_lastfrost_tree[n]-frostgdd0) 
        + beta_lambda2_spring * (spring_temps_tree[n]-springtemp0);
      
        seed_counts_new_brkd[global_idx] = neg_binomial_2_alt_log_safe_rng(loglambda2, psi2); // neg_binomial_alt_rng(lambda2, psi2) 
      }
      
      seed_counts_new_brkd_plot[newtree_stand_idxs[newt], n] += seed_counts_new_brkd[global_idx];
    }
  }
}
